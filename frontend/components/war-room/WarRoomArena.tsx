...
// synthesis is a single separate call after turn 9 (see runSynthesis).
...
// base = structural (ring / bubble top accent); text = label tint on dark;
// fill = circle wash; pos = % position inside the responsive arena box.
...
export function WarRoomArena({ id }: { id: string }) {
  ...
  // Follow the debate as turns reveal.
  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" });
  }, [messages.length, activeAgent, phase]);

  // ── Persistence (best-effort; the debate keeps running if a save fails) ─────
  ...
  // ── Map synthesis (single call over the full transcript) ────────────────────
  ...
  // ── Debate loop (sequential; passes the full transcript-so-far each turn) ────
  const runFrom = useCallback(
    async (
      start: number,
      initial: DebateMessage[],
      idea: string,
      responses: QA[]
    ) => {
      setError(null);
      ...
      for (let i = start; i < DEBATE_STEPS.length; i++) {
        ...
        try {
          ...
          // Persist after each completed round (idempotent on the backend).
          if ((i + 1) % 3 === 0) {
  ...
  const init = useCallback(async () => {
    ...
    try {
      ...
      const responses: QA[] = Array.isArray(data?.questionnaireResponses)
        ? data.questionnaireResponses
        : [];
      setIdeaSummary(idea);
      setQuestionnaire(responses);

      // Rebuild the transcript in canonical step order; stop at the first gap.
      ...
      // All nine turns exist but no map yet → run synthesis only.
  ...
  // Run once (guard against React strict-mode double-invoke — turns are costly).
  useEffect(() => {
  ...
  // Full-stage load / load-error before we have any content.
  if (phase === "loading") {
    return (
      <Stage>
        <div className="flex flex-1 flex-col items-center justify-center gap-4 py-24">
          <Loader2 className="h-5 w-5 animate-spin" style={{ color: "#c2692a" }} />
          <p style={{ ...eyebrow }} className="font-mono">
            Convening the room…
          </p>
        </div>
      </Stage>
      ...
      <AnimatePresence mode="wait">
        {phase === "ready" ? (
          ...
          <motion.div
            ...
            className="flex flex-1 flex-col"
          >
            {/* Header: round stepper + idea context */}
...
// ── The roundtable (exact geometry from inspo.html, SVG viewBox 1200×800) ──────
function Arena({ activeAgent }: { activeAgent: AgentRole | null }) {
  const nodes = (Object.keys(AGENT_META) as AgentRole[]).map((role) => ({
    role,
    ...AGENT_META[role],
  }));
  ...
  return (
    <div className="mx-auto w-full max-w-[820px] px-8">
      <svg viewBox="0 0 1200 800" className="block w-full" role="img" aria-label="War Room debate arena">
        ...
        {/* Founder (present but passive in Phase 6) */}
...
// ── Round stepper (3 pills, past / current / future) ──────────────────────────
function RoundStepper({ current }: { current: 1 | 2 | 3 }) {
  return (
...
// ── Speech bubble (design-system recipe) ──────────────────────────────────────
...
function TypingBubble({ agent, round }: { agent: AgentRole; round: 1 | 2 | 3 }) {
...
// ── Ready interstitial (cross-faded in; the map itself lands in Phase 7) ───────
function ReadyInterstitial({ assumptionCount }: { assumptionCount: number }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center px-8 py-24 text-center">
      ...
      <h1
        ...
        style={{ fontSize: "34px", color: "#ede9e0", lineHeight: 1.12, marginBottom: "14px" }}
      >
        Your assumption map is ready.
      </h1>
      <p style={{ fontSize: "15px", color: "#9a958c", maxWidth: "30rem", lineHeight: 1.6 }}>
        {assumptionCount > 0
          ? `Three advisors surfaced ${assumptionCount} assumption${
              assumptionCount === 1 ? "" : "s"
            } across three rounds of debate. The Launchpad will help you act on them.`
      ...
      <p
        ...
        className="font-serif"
      >
        This reflects only what you told the room — it doesn&apos;t replace talking to real
        customers.
      </p>
      ...
      <span className="mt-3 font-mono" style={eyebrow}>
...
// ── Errors ────────────────────────────────────────────────────────────────────
function TurnError({ message, onRetry }: { message: string | null; onRetry: () => void }) {
  return (
    <div
      style={{
        background: "rgba(194,105,42,0.08)",
        border: "1px solid rgba(194,105,42,0.40)",
        borderRadius: "13px",
        padding: "15px 18px",
      }}
    >
