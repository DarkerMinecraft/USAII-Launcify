...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (err instanceof GeminiParseError) {
      return NextResponse.json({ error: "Could not parse the executive summary" }, { status: 502 });
    }
    return NextResponse.json({ error: "An error occurred generating the summary" }, { status: 500 });
  }
}
